...
def _svo_keep(text: str) -> bool:
    """Строгий фильтр для СВО: нужна комбинация из 2+ групп сигналов + чёрный список."""
    t = (text or "").lower()
    ...
    # ✅ Основные ключи (расширены: США, Америка, Зеленский)
    core = [
        "сво", "спецоперац", "лбс", "фронт", "военн", "сводк", "минобороны",
        "всу", "зсу", "бригада", "батальон", "полк", "артилл", "мином",
        "пво", "бпла", "дрон", "шахед", "герань", "ракет", "танк",
        "бронетех", "боеприпас", "окоп", "инженерн",
        "сша", "америка", "зеленский", "путин"
    ]
    ...
    score = 0
    if any(k in t for k in core):    score += 1
    if any(k in t for k in actions): score += 1
    if any(k in t for k in places):  score += 1
