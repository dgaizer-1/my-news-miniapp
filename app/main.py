...
def _svo_keep(text: str) -> bool:
    """Оставляем только реально военные новости."""
    t = (text or "").lower()

    # ключевые маркеры темы
    keep = [
        "сво", "лбс", "фронт", "военн", "сводк", "минобороны",
        "всу", "зсу", "бригада", "батальон", "полк",
        "наступлен", "контрнаступ", "оборона", "прорыв", "рейд", "окоп",
        "артилл", "мином", "обстрел", "удар", "пво",
        "ракет", "дрон", "бпла", "шахед", "герань",
        "танк", "бронетех", "боеприпас",
        # частые топонимы/направления (можно расширять по мере надобности)
        "бахмут", "авдеев", "купянск", "запорож", "херсон", "лиман", "сватово"
    ]
